...
# Default configs
TOOLS_LIST = ['google-serper', 'wolfram-alpha', 'wikipedia'] # news-api
TOOLS_DEFAULT_LIST = ['google-serper']
...
with gr.Blocks(title="Chat-GPT-Enhance", theme=gr.themes.Soft()) as block:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            gr.HTML("""<b><center>Tools and API Keys</center></b>""")
            ...
            apply.click(chat.update_settings, inputs=[use_gpt4, tools_group, serper_api_key, wolfram_alpha_appid],
    ...
    message.submit(chat_wrapper, inputs=[openai_api_key, message, history_state, chain_state, trace_chain,
                                         monologue, express_chain_state, translate_to, force_translate],
                   outputs=[chatbot, history_state, message])

    submit.click(chat_wrapper, inputs=[openai_api_key, message, history_state, chain_state, trace_chain,
                                         monologue, express_chain_state, translate_to, force_translate],
                 outputs=[chatbot, history_state, message])
